...
ParseResult FIRCircuitParser::parseTypeDecl() {
  ...
  // TODO: Currently type alias cannot be considered as a base type so most case
  //       will fail. Once the issue was fixed, replace `type`
  //       with `FTypeAliasType::get(name, type)`.
  if (!getConstants().aliasMap.insert({id, type}).second)
    return emitError() << "type alias " << name << " is already defined";
  return success();
}
