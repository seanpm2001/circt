...
static LogicalResult customTypePrinter(Type type, AsmPrinter &os) {
  ...
  TypeSwitch<Type>(type)
    ...
    os << "enum<";
    llvm::interleaveComma(fenumType, os, [&](FEnumType::EnumElement element) {
      os << element.name.getValue();
      os << ": ";
      printNestedType(element.type, os);
    });
...
static OptionalParseResult customTypeParser(AsmParser &parser, StringRef name,
    ...
    if (name.equals("openbundle")) {
      ...
      return result = OpenBundleType::get(context, elements, isConst),
             success();
    }
    ...
    if (name.equals("enum")) {
      ...
      if (parser.parseCommaSeparatedList(
              mlir::AsmParser::Delimiter::LessGreater, parseEnumElement))
        return failure();
    ...
    if (name.equals("openvector")) {
      ...
      return result = OpenVectorType::get(elementType, width, isConst),
             success();
    }
...
bool FIRRTLType::isGround() {
    return TypeSwitch<FIRRTLType, bool>(*this)
        ...
        .Case<BundleType, FVectorType, FEnumType, OpenBundleType,
              OpenVectorType>([](Type) { return false; })
        .Case<BaseTypeAliasType>([](BaseTypeAliasType alias) {
    ...
    return TypeSwitch<FIRRTLType, RecursiveTypeProperties>(*this)
        ...
        .Case<SIntType, UIntType>([](auto type) {
          return RecursiveTypeProperties{
              true, false, false, type.isConst(), true, !type.hasWidth(),
              false};
        })
        ...
        .Case<BundleType, FVectorType, FEnumType, OpenBundleType,
              OpenVectorType, RefType, BaseTypeAliasType>(
...
FIRRTLBaseType FIRRTLBaseType::getConstType(bool isConst) {
    return TypeSwitch<FIRRTLBaseType, FIRRTLBaseType>(*this)
        ...
        .Case<BaseTypeAliasType>([&](auto type) {
          return type.getInnerType().getConstType(isConst);
        })
        .Default([](Type) {
          llvm_unreachable("unknown FIRRTL type");
          return FIRRTLBaseType();
...
bool firrtl::areTypesEquivalent(FIRRTLType destFType, FIRRTLType srcFType,
    ...
    if (destVectorType && srcVectorType)
      return destVectorType.getNumElements() ==
    ...
    auto srcEnumType = firrtl::type_dyn_cast<FEnumType>(destType);
    ...
    if (destBundleType && srcBundleType)
      return llvm::all_of(destBundleType, [&](auto destElt) -> bool {
        ...
        return areTypesWeaklyEquivalent(
            ...
            srcFlip ^ srcElt->isFlip, destOuterTypeIsConst,
            srcOuterTypeIsConst);
      });
...
Type firrtl::getPassiveType(Type anyBaseFIRRTLType) {
    return firrtl::type_cast<FIRRTLBaseType>(anyBaseFIRRTLType)
        .getPassiveType();
}
...
SIntType SIntType::get(MLIRContext *context) {
    return get(context, -1, false); }
...
int32_t SIntType::getWidthOrSentinel() const {
...
int32_t UIntType::getWidthOrSentinel() const {
    return getImpl()->width; }
...
uint64_t BundleType::getMaxFieldID() {
...
FIRRTLBaseType BundleType::getAnonymousType() {
    ...
    for (auto &elt : impl->elements)
      newElements.push_back(
          {elt.name, elt.isFlip, elt.type.getAnonymousType()});
...
uint64_t OpenBundleType::getMaxFieldID() {
    return getImpl()->maxFieldID; }
...
size_t FVectorType::getNumElements() const {
...
size_t OpenVectorType::getNumElements() const {
    return getImpl()->numElements; }
...
uint64_t FEnumType::getMaxFieldID() {
    return getImpl()->maxFieldID; }
...
auto FEnumType::verify(function_ref<InFlightDiagnostic()> emitErrorFn,
    ...
    for (auto &elt : elements) {
      ...
      if (r.containsAnalog)
        return emitErrorFn()
               << "enum field '" << elt.name << "' contains analog";
...
int32_t AnalogType::getWidthOrSentinel() const {
    return getImpl()->width; }
...
void FIRRTLDialect::registerTypes() {
    addTypes<SIntType, UIntType, ClockType, ResetType, AsyncResetType,
             AnalogType,
             // Derived Types
