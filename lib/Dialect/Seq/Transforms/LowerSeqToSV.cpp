...
// Check that for all index i, `nextReg[i]` is equal to
// `(cond & idx == i) ? val : reg[i]`. If so, we can construct transform the
// entire expression into the assignment to dynamic array indexing. if (cond)
//
// if (cond)
//   reg[idx] <= val;
//
