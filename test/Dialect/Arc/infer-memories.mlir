...
hw.module @TestWOMemory(%clock: i1, %addr: i10, %enable: i1, %data: i8) {
  ...
  // CHECK-NEXT: [[FOO:%.+]] = arc.memory <1024 x i8>
  // CHECK-NEXT: arc.state_tap [[FOO]] memory rw "foo" : !arc.memory<1024 x i8>
...
hw.module @TestWOMemoryWithMask(%clock: i1, %addr: i10, %enable: i1, %data: i16, %mask: i2) {
  // CHECK-NOT: hw.instance
  // CHECK-NEXT: [[FOO:%.+]] = arc.memory <1024 x i16>
  // CHECK-NEXT: arc.state_tap [[FOO]] memory rw "foo" : !arc.memory<1024 x i16>
